...
function PostCard({ post, onDelete, authorId }) {
    ...
    const fetchIsFollowing = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8000/api/v1/users/isFollowing/${authorId}`
        );
  ...
  const followUser = async (event) => {
    ...
    try {
      ...
      await axios.post(`http://localhost:8000/api/v1/users/follow/${authorId}`);
    } catch (error) {
      toast({
